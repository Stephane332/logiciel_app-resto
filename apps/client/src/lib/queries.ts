...
export const useRestaurant = () =>
  useQuery({ queryKey: ['restaurant'], queryFn: api.restaurant, staleTime: 5 * 60_000 });
...
export const useMyOrders = (enabled: boolean) =>
  useQuery({ queryKey: ['orders', 'mine'], queryFn: api.myOrders, enabled, staleTime: 15_000 });
...
export const useAddresses = (enabled: boolean) =>
  useQuery({ queryKey: ['addresses'], queryFn: api.addresses, enabled });

export const useNotifications = (enabled: boolean) =>
  useQuery({ queryKey: ['notifications'], queryFn: api.notifications, enabled, staleTime: 30_000 });

export const useLoyalty = (enabled: boolean) =>
  useQuery({ queryKey: ['loyalty'], queryFn: api.loyalty, enabled });
