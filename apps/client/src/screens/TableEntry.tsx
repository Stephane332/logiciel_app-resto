      ...
      <EmptyState
        ...
        description={
          error?.isOffline
            ...
            : (error?.message ?? 'Demandez au personnel de vérifier le code de la table.')
        }
        action={
          <div className="row" style={{ gap: 'var(--space-2)' }}>
            <button type="button" className="btn btn--secondary" onClick={() => resolve.mutate()}>
              Réessayer
            </button>
  ...
  return (
    <div style={{ display: 'grid', placeItems: 'center', minHeight: '60dvh' }}>
      <div className="empty-state">
        <div className="empty-state__icon" style={{ color: 'var(--brand-primary)' }}>
          <IconQr size={28} />
        </div>
        <p className="section-title">Reconnaissance de votre table…</p>
        <Loading rows={0} />
      </div>
    </div>
  );
